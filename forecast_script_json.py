...
def data_get(wd):
    ...
    start_url_1_to_3 = "https://tgftp.nws.noaa.gov/SL.us008001/ST.opnl/DF.gr2/DC.ndfd/AR.conus/VP.001-003/"
    start_url_4_to_7 = "https://tgftp.nws.noaa.gov/SL.us008001/ST.opnl/DF.gr2/DC.ndfd/AR.conus/VP.004-007/"
...
def generate_json(wd):
    ...
    def process_grib_file(bin_file, forecast_type):
        ...
        for grb in grbs:
            ...
            forecast_date = grb.validDate.date()
            units = grb.units
            units = units.replace(" ", "")
            ...
            # ---- MINIMAL FIX: nearest gridpoint instead of exact equality ----
            # Use squared distance in lat/lon degrees (fine for nearest index selection)
            ...
            if np.ma.is_masked(v):
                # Skip this grb message (or set None). Skipping is safest.
                continue
    ...
    if os.path.exists(json_path):
        try:
            ...
            print("Error: Existing JSON file is corrupted. Creating a new one.")
            with open(json_path, 'w') as json_file:
...
if __name__ == "__main__":
    wd = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'weatherdata/news_refs')
    data_get(wd)
    generate_json(wd)
